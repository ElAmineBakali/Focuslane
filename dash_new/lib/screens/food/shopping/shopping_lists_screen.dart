    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () async {
                    ...
                    children: [
                      ...
                      DropdownButton<ShoppingScope>(
                        ...
                        items: const [
                          ...
                          DropdownMenuItem(value: ShoppingScope.biweekly, child: Text('Quincenal')),
                          DropdownMenuItem(value: ShoppingScope.monthly, child: Text('Mensual')),
                        ],
              ...
              if (ok == true) {
                ...
                Navigator.push(context, MaterialPageRoute(builder: (_) => ShoppingListDetailScreen(svc: svc, listId: id)));
              }
            },
          ),
        ],
      ...
      body: StreamBuilder<List<ShoppingList>>(
        ...
        builder: (context, snap) {
          ...
          if (lists.isEmpty) return const Center(child: Text('Crea tu primera lista con +'));
            ...
            itemBuilder: (_, i) {
              ...
              return Card(
                child: ListTile(
                  ...
                  title: Text(l.name),
                  subtitle: Text('${l.scope.name} • ${l.items.length} items'
                      '${total > 0 ? ' • Estimado: ${total.toStringAsFixed(2)}' : ''}'),
                  trailing: PopupMenuButton<String>(
                    onSelected: (v) async {
                      if (v == 'default') await svc.setDefaultList(l.id);
                      if (v == 'delete') {
                        final ok = await _confirm(context, 'Eliminar', '¿Eliminar "${l.name}"?');
                        if (ok) await svc.deleteShoppingList(l.id);
                      }
                    },
                    itemBuilder: (_) => const [
                      PopupMenuItem(value: 'default', child: Text('Marcar como predeterminada')),
                      PopupMenuItem(value: 'delete', child: Text('Eliminar')),
                    ],
                  ),
                  onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => ShoppingListDetailScreen(svc: svc, listId: l.id)),
                  ),
                ),
              );
            },
