    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<PantryItem>>(
        ...
        builder: (context, snap) {
          ...
          if (list.isEmpty)
            return const Center(child: Text('Vacío. Añade productos con +'));
          return ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return Card(
                child: ListTile(
                  leading: Icon(
                    low ? Icons.warning_amber : Icons.kitchen,
                    color:
                        low
                            ? Colors.amber
                  ...
                  title: Text(p.name),
                  subtitle: Text(
                    '${p.qty.toStringAsFixed(0)} ${p.unit.name}${p.minQty != null ? ' • mínimo ${p.minQty}' : ''}',
                  ),
                  trailing: PopupMenuButton<String>(
                    onSelected: (v) async {
                      if (v == 'consume') {
                        final q = await _promptNumber(
                          context,
                          'Consumir cantidad',
                        );
  ...
  Future<void> _editItem(
    ...
    final ok = await showDialog<bool>(
      ...
      builder:
          (_) => AlertDialog(
            title: Text(
              initial == null ? 'Añadir a despensa' : 'Editar despensa',
            ),
            content: Column(
