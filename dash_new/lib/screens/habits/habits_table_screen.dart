...
class _HabitsTableScreenState extends State<HabitsTableScreen> {
  ...
  late final List<DateTime> _dates =
      List.generate(30, (i) => DateTime.now().subtract(Duration(days: i)));
  ...
  void initState() {
    ...
    _leftV.addListener(() {
      ...
      _syncingL = true;
      final sharedMax = math.min(_leftV.position.maxScrollExtent, _rightV.position.maxScrollExtent);
  ...
  Widget _valueContent(Habit habit, dynamic value, ThemeData theme) {
    if (value == '✔️') return Icon(Icons.check_rounded, color: habit.color, size: 22);
    if (value == '❌') return Icon(Icons.close_rounded, color: habit.color, size: 22);
    ...
    if (habit.isQuantitative && isNum) {
      return Column(
        ...
        children: [
          Text(s, style: TextStyle(color: habit.color, fontWeight: FontWeight.bold)),
          if (habit.unit.isNotEmpty) const SizedBox(height: 2),
          if (habit.unit.isNotEmpty)
            Text(habit.unit, style: TextStyle(fontSize: 10, color: theme.colorScheme.onSurface.withOpacity(.6))),
        ],
      );
    }
    return Text('–', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(.45)));
  }
    ...
    if (!habit.isQuantitative) {
      final result = await showDialog<String>(
        ...
        builder: (_) => AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, '✔️'), style: TextButton.styleFrom(foregroundColor: sec), child: const Icon(Icons.check_rounded)),
            TextButton(onPressed: () => Navigator.pop(context, '❌'), style: TextButton.styleFrom(foregroundColor: sec), child: const Icon(Icons.close_rounded)),
            TextButton(onPressed: () => Navigator.pop(context, '-'), style: TextButton.styleFrom(foregroundColor: sec), child: const Text('Saltar')),
          ],
        ),
      );
      if (result != null) {
        ...
        if (result == '✔️' && _isToday(date)) {
          ...
          if (mounted) {
            ...
            final allCompleted = allHabits.every((h) => h.history[todayKey] == '✔️');
      ...
      final result = await showDialog<String>(
        ...
        builder: (_) => AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, null), style: TextButton.styleFrom(foregroundColor: sec), child: const Text('Cancelar')),
            TextButton(onPressed: () => Navigator.pop(context, controller.text), style: TextButton.styleFrom(foregroundColor: sec), child: const Text('Guardar')),
            TextButton(onPressed: () => Navigator.pop(context, '-'), style: TextButton.styleFrom(foregroundColor: sec), child: const Text('Saltar')),
          ],
        ),
      );
      if (result != null) {
        ...
        if (result != '-' && _isToday(date)) {
          ...
          final goalMet = value > 0; // Cualquier valor > 0 cuenta como completado
  ...
  Future<void> _updateStreak(Habit habit, bool completed) async {
    if (completed) {
      ...
      if (habit.currentStreak > 0) {
        await HabitFirestoreService.updateHabitFields(habit.id, {'currentStreak': 0});
        ...
        children: [
          ListTile(
            ...
            onTap: () async {
              ...
              final result = await Navigator.pushNamed(context, '/habits/detail', arguments: h);
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final stream = _showArchived ? HabitFirestoreService.getArchivedHabits() : HabitFirestoreService.getHabits();

    final double gridWidth = _dates.length * (_cellWidth + _cellMargin.horizontal);

    return Scaffold(
      backgroundColor: Theme.of(context).brightness == Brightness.light ? const Color.fromARGB(255, 255, 255, 255) : Theme.of(context).scaffoldBackgroundColor,
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<String>(
            ...
            onSelected: (value) async {
              if (value == 'schedule') {
                final picked = await showTimePicker(context: context, initialTime: const TimeOfDay(hour: 21, minute: 0));
                if (picked != null) {
                  // ⚠️ Para que funcione al 100% necesito ver NotificationService.dart
                  await NotificationService.I.scheduleHabitDailyReminder(picked);
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(behavior: SnackBarBehavior.floating, content: Text('👌 Te avisaré todos los días a la hora elegida para revisar hábitos.')),
                    );
                  }
                }
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(behavior: SnackBarBehavior.floating, content: Text('⏰ Recordatorio diario cancelado.')),
                  );
                }
              }
            },
            itemBuilder: (context) => const [
              PopupMenuItem(value: 'schedule', child: Text('Programar recordatorio')),
              PopupMenuItem(value: 'cancel', child: Text('Cancelar recordatorio')),
            ],
          ),
          IconButton(
            ...
            icon: Icon(_showArchived ? Icons.inbox_rounded : Icons.archive_outlined),
          ),
          if (_editMode && _selectedHabit != null) IconButton(icon: const Icon(Icons.more_vert), onPressed: _showEditOptions),
          if (_editMode) IconButton(icon: const Icon(Icons.close), onPressed: _exitEditMode),
        ],
      ),
      ...
      body: SafeArea(
        ...
        child: StreamBuilder<List<Habit>>(
          ...
          builder: (context, snap) {
            ...
            final habits = [...snap.data!..sort((a, b) => a.order.compareTo(b.order))];
                                    ...
                                    return _NameRow(
                                      ...
                                      trailing: ReorderableDragStartListener(
                                        ...
                                        child: const Padding(
                                          ...
                                          child: Icon(Icons.drag_handle_rounded, size: 18, color: Colors.grey),
                                        ),
                                      ),
                                    );
                                  },
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return _NameRow(
                                    habit: habit,
                                    editMode: false,
                                    onTap: () => Navigator.pushNamed(context, '/habits/stats', arguments: habit),
                                  );
                                },
                              ),
                      ),
                                  ...
                                  children: _dates.map((d) {
                                    return Container(
                                      ...
                                      child: Text(
                                        ...
                                        style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(.8)),
                                      ),
                                    );
                                  }).toList(),
                                ),
                              ...
                              SizedBox(
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, row) {
                                    ...
                                    return Row(
                                      children: _dates.map((d) => _cell(habit, d, theme)).toList(),
                                    );
                                  },
                                ),
                              ),
        ...
        children: [
          Expanded(
            child: GestureDetector(
              ...
              onLongPress: () {
                if (!editMode) {
                  final state = context.findAncestorStateOfType<_HabitsTableScreenState>();
                ...
                child: Row(
                  children: [
                    ...
                    if (habit.emoji != null)
                      ...
                      Icon(HabitIcons.getIcon(habit.iconCode), color: habit.color, size: 20)
                    else
                      Icon(Icons.check_circle_outline, color: habit.color, size: 20),
                    ...
                    Expanded(
                      child: Text(
                        ...
                        style: TextStyle(fontWeight: FontWeight.w600, color: habit.color, fontSize: 13),
                      ),
                    ),
                  ],
                ),
