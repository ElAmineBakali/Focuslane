...
class _TransactionFormScreenState extends State<TransactionFormScreen> {
  ...
  String _divisa = 'EUR';
  double _fxRate = 1.0;
  String _recurrence = 'once'; // once, daily, weekly, monthly, yearly
  String? _envelopeId;
  
  final _categories = {
    TxType.income: ['Salario', 'Freelance', 'Inversiones', 'Otros Ingresos'],
    TxType.expense: ['Alimentación', 'Transporte', 'Vivienda', 'Ocio', 'Salud', 'Educación', 'Compras', 'Otros Gastos'],
  };
  ...
  final _subCategories = {
    ...
    'Vivienda': ['Alquiler', 'Hipoteca', 'Electricidad', 'Agua', 'Internet', 'Reparaciones'],
    'Ocio': ['Cine', 'Conciertos', 'Viajes', 'Hobbies', 'Streaming'],
    'Salud': ['Médico', 'Farmacia', 'Gimnasio', 'Seguros'],
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    _buildCategoryField(),
                    if (_category != null && _subCategories.containsKey(_category)) ...[
                      const SizedBox(height: 16),
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Tipo de transacción', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600)),
                ...
                Expanded(
                  child: ChoiceChip(
                    label: Row(
                      ...
                      children: [
                        Icon(Icons.trending_up, size: 18, color: _type == TxType.income ? Colors.white : FinanceUI.income),
                    ...
                    selectedColor: FinanceUI.income,
                  ),
                ),
                ...
                Expanded(
                  child: ChoiceChip(
                    label: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.trending_down, size: 18, color: _type == TxType.expense ? Colors.white : FinanceUI.expense),
                        const SizedBox(width: 8),
                        const Text('Gasto'),
                      ],
                    ),
                    ...
                    selectedColor: FinanceUI.expense,
                  ),
                ),
              ],
            ),
      ...
      children: [
        ...
        Expanded(
          child: DropdownButtonFormField<String>(
            ...
            items: _divisas.map((d) => DropdownMenuItem(value: d, child: Text(d))).toList(),
  ...
  Widget _buildDateField() {
    return InkWell(
      ...
      child: InputDecorator(
        ...
        child: Text(
          DateFormat('EEEE, d MMM yyyy HH:mm', 'es').format(_date),
          style: GoogleFonts.poppins(),
        ),
      ),
    );
  ...
  Widget _buildSubCategoryField() {
    ...
    return DropdownButtonFormField<String>(
      ...
      items: subs.map((sub) => DropdownMenuItem(value: sub, child: Text(sub))).toList(),
  ...
  Widget _buildAccountField() {
    return TextFormField(
      ...
      decoration: InputDecoration(
        labelText: 'Cuenta/Método de pago',
        hintText: 'Ej: Tarjeta débito, Efectivo',
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              widget.transaction == null ? 'Transacción creada' : 'Transacción actualizada',
              style: GoogleFonts.poppins(),
            ),
          ),
        );
