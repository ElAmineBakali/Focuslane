...
class _FinanceSettingsScreenState extends State<FinanceSettingsScreen> {
  ...
  DocumentReference<Map<String, dynamic>> get _metaDoc => FirebaseFirestore
      .instance
      .collection('users')
      .doc(_uid)
      .collection('finance')
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() {
        ...
        _budgetsAlertPct =
            ((notif['budgetsThresholdPct'] ?? 80) as num).toDouble();
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              const Expanded(
                                child: Text('Inicio de mes financiero'),
                              ),
                              DropdownButton<int>(
                          ...
                          SwitchListTile(
                            title: const Text(
                              'Bloqueo con PIN/Biometría (solo módulo de finanzas)',
                            ),
                            value: _pinLock,
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            trailing: SizedBox(
                              ...
                              child: TextField(
                                ...
                                controller: TextEditingController(
                                  text: _budgetsAlertPct.toStringAsFixed(0),
                                ),
                                onChanged: (v) {
                                  final x = double.tryParse(v);
                                  if (x != null)
                                    _budgetsAlertPct = x.clamp(0, 100);
                                },
                              ),
                            ),
                          ),
                          ListTile(
                            contentPadding: EdgeInsets.zero,
                            title: const Text(
                              'Recordar gastos fijos (días antes)',
                            ),
                            trailing: DropdownButton<int>(
                              value: _fixedRemindDays,
                              onChanged:
                                  (v) =>
                                      setState(() => _fixedRemindDays = v ?? 3),
                              items:
