                ...
                return ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      ...
                      title: Text(it.name),
                      subtitle: Text('${it.category} • ${it.status}${it.amount != null ? " • ${it.amount!.toStringAsFixed(2)}" : ""}'),
                      trailing: PopupMenuButton<String>(
                        onSelected: (v) async {
                          if (v == 'done') {
                            await svc.updateVariableExpense(periodKey, VariableExpenseItem(
                              id: it.id, name: it.name, category: it.category, periodKey: it.periodKey, status: 'done', amount: it.amount, linkedTxId: it.linkedTxId,
                            ));
                          } else if (v == 'planned') {
                            await svc.updateVariableExpense(periodKey, VariableExpenseItem(
                              id: it.id, name: it.name, category: it.category, periodKey: it.periodKey, status: 'planned', amount: it.amount, linkedTxId: it.linkedTxId,
                            ));
                          } else if (v == 'delete') {
                        ...
                        itemBuilder: (c) => const [
                          PopupMenuItem(value: 'planned', child: Text('Marcar planificado')),
                          PopupMenuItem(value: 'done', child: Text('Marcar realizado')),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                SizedBox(width: 120, child: TextField(controller: _category, decoration: const InputDecoration(labelText: 'Categoría'))),
                const SizedBox(width: 8),
                SizedBox(width: 110, child: TextField(controller: _amount, decoration: const InputDecoration(labelText: '€ (opcional)'))),
                const SizedBox(width: 8),
                FilledButton(
                  onPressed: () async {
                    if (_name.text.trim().isEmpty) return;
                    await svc.addVariableExpense(periodKey, VariableExpenseItem(
                      id: '',
