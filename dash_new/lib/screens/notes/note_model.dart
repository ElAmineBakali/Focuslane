...
class Note {
  ...
  factory Note.fromDoc(DocumentSnapshot doc) {
    ...
    final spans = (spansRaw is List)
      ? spansRaw.map((e) => FormatSpan.fromMap(Map<String, dynamic>.from(e))).toList()
      : <FormatSpan>[];
    final delta = (data['delta'] is List)
      ? List<dynamic>.from(data['delta'])
      : null;
    final attachmentsRaw = data['attachments'];
    final attachments = (attachmentsRaw is List)
      ? attachmentsRaw.map((e) => NoteAttachment.fromMap(Map<String, dynamic>.from(e))).toList()
      : <NoteAttachment>[];
    ...
    return Note(
      ...
      linkedTaskIds: (data['linkedTaskIds'] as List?)?.map((e) => e.toString()).toList() ?? const [],
