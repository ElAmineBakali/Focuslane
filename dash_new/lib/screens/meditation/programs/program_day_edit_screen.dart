...
class _ProgramDayEditScreenState extends State<ProgramDayEditScreen> {
  ...
  Widget build(BuildContext context) {
    if (program == null) return const Scaffold(body: Center(child: Text('Sin programa')));
        ...
        child: Padding(
          ...
          child: Form(
            ...
            child: ListView(
              ...
              TextFormField(
                controller: _goal,
                decoration: const InputDecoration(labelText: 'Objetivo / guía'),
              ),
              const SizedBox(height: 8),
              Row(
                children: [
              ...
              Row(
                children: [
                  ...
                  DropdownButton<int>(
                    ...
                    onChanged: (v) => setState(() => _recommendedMin = v ?? 10),
                    items: [5,10,15,20,25,30,40,45,60].map((e) => DropdownMenuItem(value: e, child: Text('$e'))).toList(),
                  ),
                ],
              ),
              ...
              StreamBuilder<List<GuidedAudio>>(
                ...
                builder: (context, s) {
                  ...
                  return DropdownButtonFormField<String?>(
                    initialValue: list.any((g) => g.id == _guidedId) ? _guidedId : null,
                    items: [
                      const DropdownMenuItem<String?>(value: null, child: Text('Sin guía')),
                      ...list.map((g) => DropdownMenuItem<String?>(value: g.id, child: Text('Guía: ${g.title}'))),
                    ],
                    onChanged: (v) => setState(() => _guidedId = v),
                    decoration: const InputDecoration(labelText: 'Guía (opcional)'),
                  );
                },
              ),
