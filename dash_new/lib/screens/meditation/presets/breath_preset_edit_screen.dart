...
class _BreathPresetEditScreenState extends State<BreathPresetEditScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (arg is BreathPreset && editing == null) {
      editing = arg;
      _name.text = arg.name;
      _inhale = arg.inhale; _hold = arg.hold; _exhale = arg.exhale; _hold2 = arg.hold2;
      _cycles = arg.cycles; _vibration = arg.vibration; _visual = arg.visualStyle;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(editing == null ? 'Nuevo preset' : 'Editar preset')),
      body: TaskFormTheme(
        child: Padding(
          ...
          child: ListView(
            children: [
              ...
              _numPicker('Inhala (s)', _inhale, (v) => setState(() => _inhale = v), [2,3,4,5,6,7,8,10]),
              _numPicker('Mantén (s)', _hold, (v) => setState(() => _hold = v), [0,2,3,4,5,6,8,10]),
              _numPicker('Exhala (s)', _exhale, (v) => setState(() => _exhale = v), [2,3,4,5,6,7,8,10]),
              _numPicker('Mantén 2 (s)', _hold2, (v) => setState(() => _hold2 = v), [0,2,3,4,5,6,8,10]),
              _numPicker('Ciclos', _cycles, (v) => setState(() => _cycles = v), [3,4,5,6,7,8,10,12]),
              SwitchListTile(title: const Text('Vibración'), value: _vibration, onChanged: (v) => setState(() => _vibration = v)),
              ...
              FilledButton.icon(
                ...
                onPressed: () async {
                  ...
                  final p = BreathPreset(
                    ...
                    inhale: _inhale, hold: _hold, exhale: _exhale, hold2: _hold2,
                    cycles: _cycles, vibration: _vibration, visualStyle: _visual,
                  );
                  if (editing == null) {
  ...
  Widget _numPicker(String label, int value, void Function(int) onChanged, List<int> options) {
    return ListTile(
