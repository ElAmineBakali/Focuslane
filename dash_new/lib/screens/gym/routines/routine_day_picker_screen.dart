...
class RoutineDayPickerScreen extends StatefulWidget {
  ...
  const RoutineDayPickerScreen({super.key, required this.svc, required this.routine});
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          ...
          if (ok == true) {
            ...
            if (name.isNotEmpty) {
              await svc.addDay(widget.routine.id, name, order: DateTime.now().millisecondsSinceEpoch);
              _dayCtrl.clear();
            }
          }
        },
        ...
        builder: (context, snap) {
          ...
          return ReorderableListView.builder(
            ...
            onReorder: (oldIndex, newIndex) async {
              ...
              await svc.reorderDays(widget.routine.id, ordered.map((d) => d.id).toList());
            },
            itemBuilder: (_, i) {
              final d = days[i];
              return Card(
                ...
                child: ListTile(
                  ...
                  title: Text(d.name),
                  subtitle: _LastDoneMini(svc: svc, routineId: widget.routine.id, dayId: d.id),
                  trailing: IconButton(
                    ...
                    onPressed: () async {
                      ...
                      if (ok == true) await svc.deleteDayCascade(widget.routine.id, d.id);
                    },
                  ),
                  onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => LiveSessionScreen(svc: widget.svc, routine: widget.routine, day: d),
                    ),
                  ),
                ),
              );
...
class _LastDoneMini extends StatelessWidget {
  ...
  final String dayId;
  const _LastDoneMini({required this.svc, required this.routineId, required this.dayId});
