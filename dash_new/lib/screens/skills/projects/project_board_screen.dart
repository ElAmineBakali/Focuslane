...
class _ProjectBoardScreenState extends State<ProjectBoardScreen> {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<Project>>(
        ...
        builder: (_, snap) {
          ...
          Widget col(String title, ProjectState state, List<Project> items) => Expanded(
            child: Card(
              child: Padding(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return Card(
                            child: ListTile(
                              title: Text(p.title),
                              subtitle: Text(
                                (p.dueDate != null ? 'Fecha: ${p.dueDate!.toLocal().toString().split(" ").first} • ' : '') +
                                (p.description.isNotEmpty ? p.description : '')
                              ),
                              trailing: PopupMenuButton<String>(
                                onSelected: (v) async {
                                  if (v.startsWith('move:')) {
                                    final st = ProjectState.values.firstWhere((e) => e.name == v.substring(5));
                                    await svc.updateProject(Project(
                                      id: p.id, skillId: p.skillId, title: p.title, description: p.description,
                                      state: st, dueDate: p.dueDate, checklist: p.checklist, evidenceUrls: p.evidenceUrls,
                                    ));
                                  } else if (v == 'edit') {
                                ...
                                itemBuilder: (_) => [
                                  ...
                                  ...ProjectState.values.map((st) =>
                                    PopupMenuItem(value: 'move:${st.name}', child: Text('Mover a ${_titleFor(st)}'))),
                                  const PopupMenuDivider(),
  ...
  String _titleFor(ProjectState s) {
    switch (s) {
      case ProjectState.idea: return 'Ideas';
      case ProjectState.doing: return 'En curso';
      case ProjectState.blocked: return 'Bloqueado';
      case ProjectState.done: return 'Hecho';
    }
  }

  void _openEditor(BuildContext context, SkillsFirestoreService svc, Skill s, {Project? editing}) {
    ...
    final state = ValueNotifier<ProjectState>(editing?.state ?? ProjectState.idea);
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            Text(editing == null ? 'Nuevo proyecto' : 'Editar proyecto',
              ...
              children: [
                Expanded(
                  child: DropdownButton<ProjectState>(
                    value: state.value,
                    items: ProjectState.values.map((e) =>
                      DropdownMenuItem(value: e, child: Text(_titleFor(e)))).toList(),
                ...
                Expanded(
                  child: ValueListenableBuilder<DateTime?>(
                    ...
                    builder: (_, d, __) => ListTile(
                      ...
                      title: Text('Fecha: ${d != null ? d.toLocal().toString().split(" ").first : "—"}'),
                      onTap: () async {
                        final pick = await showDatePicker(
                          context: context, initialDate: d ?? DateTime.now(),
