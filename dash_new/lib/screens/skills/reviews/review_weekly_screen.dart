...
class _ReviewWeeklyScreenState extends State<ReviewWeeklyScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Revisión semanal${skill != null ? ' • ${skill!.name}' : ''}')),
      body: skill == null
          ? const Center(child: Text('Abre desde una habilidad para ver su revisión'))
                ...
                return ListView(
                  ...
                  children: [
                    Wrap(
                      ...
                      children: [
                        _kpi('Horas', (m['totalHours'] ?? 0.0).toStringAsFixed(1)),
                        _kpi('Días activos', '${m['activeDays'] ?? 0}'),
                        _kpi('Sesiones', '${m['sessions'] ?? 0}'),
                      ],
                    ),
                    const SizedBox(height: 16),
                    Text('Preguntas gatillo', style: Theme.of(context).textTheme.titleLarge),
                    const SizedBox(height: 8),
                    const Text('• ¿Qué desbloqueaste esta semana?\n'
