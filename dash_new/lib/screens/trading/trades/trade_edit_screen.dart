...
class _TradeEditScreenState extends State<TradeEditScreen> {
  ...
  final _stopDist = TextEditingController(); // precio de stop (distancia absoluta o SL)
  double? _calcSize;
          ...
          child: ListView(
            children: [
              Row(
                children: [
                  ...
                  DropdownButton<AssetClass>(
                    value: _asset,
                    items: AssetClass.values.map((a)=>DropdownMenuItem(value:a, child: Text(a.name))).toList(),
                    onChanged: (v)=>setState(()=>_asset=v??_asset),
                  ),
                  const SizedBox(width: 8),
                  DropdownButton<Direction>(
                    value: _dir,
                    items: Direction.values.map((a)=>DropdownMenuItem(value:a, child: Text(a.name))).toList(),
              ...
              Row(
                children: [
                  Expanded(child: TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Precio entrada'),
                  ...
                  Expanded(child: TextFormField(
                    controller: _size,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Tamaño (unidades)'),
                    validator: (v)=> double.tryParse(v??'')==null ? 'Inválido' : null,
                  )),
                ],
              ),
              Row(
                children: [
                  Expanded(child: TextFormField(
                    controller: _sl,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Stop Loss (precio)'),
                  )),
                  const SizedBox(width: 8),
                  Expanded(child: TextFormField(
                    controller: _tp,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Take Profit (precio)'),
                  )),
                ],
              ),
              Row(
                children: [
                  Expanded(child: TextFormField(
                    controller: _exit,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Precio salida (si cerrado)'),
                  )),
                  const SizedBox(width: 8),
                  Expanded(child: TextFormField(
                    controller: _fees,
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Comisiones'),
                  )),
                ],
              ),
              const SizedBox(height: 8),
              ListTile(
                ...
                title: Text('Entrada: ${_entryDate.toLocal().toString().split(" ").first}'),
                onTap: () async {
              ...
              ListTile(
                ...
                title: Text('Salida: ${_exitDate!=null ? _exitDate!.toLocal().toString().split(" ").first : "—"}'),
                onTap: () async {
                  final d = await showDatePicker(
                    context: context, initialDate: _exitDate ?? DateTime.now(),
                    firstDate: DateTime(2020), lastDate: DateTime(2100),
                  );
              ...
              Text('Calculadora de riesgo', style: Theme.of(context).textTheme.titleMedium),
              Row(
                children: [
              ...
              Row(
                children: [
                  Expanded(child: TextField(
                    ...
                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Stop (precio stop)'),
                  )),
                  const SizedBox(width: 8),
                  FilledButton(
                    onPressed: _calcRisk,
              ...
              if (_calcSize != null || _expR != null)
                Padding(
                  ...
                  child: Text('Tamaño sugerido: ${_calcSize?.toStringAsFixed(2) ?? "-"}  •  R esperado: ${_expR?.toStringAsFixed(2) ?? "-"}'),
                ),

              const Divider(height: 24),

              TextField(controller: _strategyId, decoration: const InputDecoration(labelText: 'Estrategia ID (opcional)')),
              TextField(controller: _tags, decoration: const InputDecoration(labelText: 'Tags (coma)')),
              ...
              FilledButton.icon(
                ...
                onPressed: () async {
                  ...
                  final t = Trade(
                    ...
                    exitPrice: _exit.text.trim().isEmpty ? null : double.parse(_exit.text),
                    size: double.parse(_size.text),
                    fees: double.tryParse(_fees.text) ?? 0,
                    stopLoss: _sl.text.trim().isEmpty ? null : double.parse(_sl.text),
                    takeProfit: _tp.text.trim().isEmpty ? null : double.parse(_tp.text),
                    strategyId: _strategyId.text.trim().isEmpty ? null : _strategyId.text.trim(),
                      ...
                      child: OutlinedButton.icon(
                        ...
                        onPressed: () async {
                          ...
                          final e = Trade(
                            ...
                            entryDate: t.entryDate, exitDate: DateTime.now(),
                            entryPrice: t.entryPrice, exitPrice: t.entryPrice, size: t.size,
                            fees: t.fees, stopLoss: t.stopLoss, takeProfit: t.takeProfit, strategyId: t.strategyId,
                            tags: t.tags, rMultiple: 0, pnl: 0, pnlPct: 0, outcome: Outcome.breakeven, notes: t.notes,
                      ...
                      child: OutlinedButton.icon(
                        ...
                        onPressed: () async {
                          if (_exit.text.trim().isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Introduce precio de salida')));
                            return;
                          }
                          final t = editing!;
                          final e = Trade(
                            id: t.id, symbol: t.symbol, assetClass: t.assetClass, direction: t.direction,
                            entryDate: t.entryDate, exitDate: DateTime.now(),
                            entryPrice: t.entryPrice, exitPrice: double.parse(_exit.text),
                            size: double.parse(_size.text), fees: double.tryParse(_fees.text) ?? t.fees,
                            stopLoss: t.stopLoss, takeProfit: t.takeProfit, strategyId: t.strategyId,
                            tags: t.tags, rMultiple: t.rMultiple, pnl: t.pnl, pnlPct: t.pnlPct, outcome: t.outcome, notes: t.notes,
                            screenshots: t.screenshots,
                          );
                          await svc.updateTrade(e);
                          if (mounted) Navigator.pop(context);
  ...
  void _calcRisk() {
    ...
    if (capital <= 0 || riskPct <= 0 || entry <= 0 || stop <= 0) {
      setState(() { _calcSize = null; _expR = null; });
      return;
    }
    ...
    setState(() { _calcSize = size as double?; _expR = expR; });
  }
}
