...
class _CandleEditScreenState extends State<CandleEditScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(editing==null ? 'Añadir vela' : 'Editar vela')),
      body: TaskFormTheme(
        child: Padding(
          ...
          child: ListView(
            children: [
              Row(
                children: [
                  ...
                  DropdownButton<Timeframe>(
                    value: _tf,
                    items: Timeframe.values.map((t)=>DropdownMenuItem(value:t, child: Text(t.code))).toList(),
              ...
              Row(
                children: [
                  Expanded(child: TextFormField(
                    controller: _open, keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Open'), validator: _v,
                  )),
                  const SizedBox(width: 8),
                  Expanded(child: TextFormField(
                    controller: _high, keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'High'), validator: _v,
                  )),
                ],
              ),
              Row(
                children: [
                  Expanded(child: TextFormField(
                    controller: _low, keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Low'), validator: _v,
                  )),
                  const SizedBox(width: 8),
                  Expanded(child: TextFormField(
                    controller: _close, keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    decoration: const InputDecoration(labelText: 'Close'), validator: _v,
                  )),
                ],
              ),
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Volumen (opcional)'),
              ),
              ...
              FilledButton.icon(
                ...
                onPressed: () async {
                  ...
                  final c = Candle(
                    ...
                    volume: _volume.text.trim().isEmpty ? null : double.tryParse(_volume.text),
                    manual: true,
                  );
                  if (editing == null) {
  ...
  String? _v(String? v) => (double.tryParse(v ?? '') == null) ? 'Inválido' : null;
}
