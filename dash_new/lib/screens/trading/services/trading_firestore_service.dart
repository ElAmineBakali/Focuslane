...
class TradingFirestoreService {
  ...
  CollectionReference get _root =>
      _db.collection('users').doc(_uid).collection('trading');
  CollectionReference get _tradesCol => _root.doc('data').collection('trades');
  CollectionReference get _strategiesCol =>
      _root.doc('data').collection('strategies');
  CollectionReference get _watchlistsCol =>
      _root.doc('data').collection('watchlists');
  CollectionReference symbolsCol(String wlId) =>
      _watchlistsCol.doc(wlId).collection('symbols');
  CollectionReference get _tagsCol => _root.doc('data').collection('tags');
  CollectionReference get _journalsCol =>
      _root.doc('data').collection('journals');
  ...
  Future<void> _touchMeta(Trade t) async {
    final monthKey =
        "${t.entryDate.year}-${t.entryDate.month.toString().padLeft(2, '0')}";
    await _metaDoc.set({
  ...
  // ===== Strategies
  Stream<List<Strategy>> watchStrategies() => _strategiesCol
      .orderBy('name')
      .snapshots()
      .map((s) => s.docs.map(Strategy.fromSnap).toList());
  Future<void> addStrategy(Strategy x) async => _strategiesCol.add(x.toMap());
  Future<void> updateStrategy(Strategy x) async =>
      _strategiesCol.doc(x.id).update(x.toMap());
  Future<void> deleteStrategy(String id) async =>
      _strategiesCol.doc(id).delete();

  // ===== Watchlists
  Stream<List<Watchlist>> watchWatchlists() => _watchlistsCol
      .orderBy('name')
      .snapshots()
      .map((s) => s.docs.map(Watchlist.fromSnap).toList());
  Stream<List<WatchSymbol>> watchSymbols(String wlId) => symbolsCol(wlId)
      .orderBy('priority')
      .snapshots()
      .map((s) => s.docs.map(WatchSymbol.fromSnap).toList());
  Future<String> addWatchlist(Watchlist w) async {
    final r = await _watchlistsCol.add(w.toMap());
    return r.id;
  }

  Future<void> updateWatchlist(Watchlist w) async =>
      _watchlistsCol.doc(w.id).update(w.toMap());
  Future<void> deleteWatchlist(String id) async =>
      _watchlistsCol.doc(id).delete();
  Future<void> addSymbol(String wlId, WatchSymbol s) async =>
      symbolsCol(wlId).add(s.toMap());
  Future<void> updateSymbol(String wlId, WatchSymbol s) async =>
      symbolsCol(wlId).doc(s.id).update(s.toMap());
  Future<void> deleteSymbol(String wlId, String id) async =>
      symbolsCol(wlId).doc(id).delete();

  // ===== Tags
  Stream<List<TradingTag>> watchTags() => _tagsCol
      .orderBy('name')
      .snapshots()
      .map((s) => s.docs.map(TradingTag.fromSnap).toList());
  Future<void> addTag(TradingTag t) async => _tagsCol.add(t.toMap());
  Future<void> deleteTag(String id) async => _tagsCol.doc(id).delete();

  // ===== Journal
  Stream<List<JournalEntry>> watchJournal() => _journalsCol
      .orderBy('date', descending: true)
      .snapshots()
      .map((s) => s.docs.map(JournalEntry.fromSnap).toList());
  Future<void> addJournal(JournalEntry j) async => _journalsCol.add(j.toMap());
  Future<void> updateJournal(JournalEntry j) async =>
      _journalsCol.doc(j.id).update(j.toMap());
  ...
  CollectionReference get _candlesCol =>
      _root.doc('data').collection('candles');
  ...
  Future<void> updateCandle(Candle c) async =>
      _candlesCol.doc(c.id).update(c.toMap());
  ...
  Future<Map<String, double>> quartilesFor({
    ...
    final candles = await fetchLastCandles(
      symbol: symbol,
      timeframe: timeframe,
      n: n,
    );
