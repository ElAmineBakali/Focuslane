    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<GradeEntry>>(
        ...
        builder: (context, snap) {
          ...
          final avg = grades.isEmpty ? 0.0 : (grades.map((g)=>g.grade).reduce((a,b)=>a+b) / grades.length);
          return ListView(
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          TextField(decoration: const InputDecoration(labelText: 'ID de tarea'), onChanged: (v)=>_taskId=v),
          TextField(decoration: const InputDecoration(labelText: 'Nota'), keyboardType: TextInputType.number, controller: _gradeCtrl),
          TextField(decoration: const InputDecoration(labelText: 'Notas (opcional)'), controller: _notesCtrl),
        ],
      ),
      actions: [
        ...
        FilledButton(onPressed: () async {
          ...
          final entry = GradeEntry(id: '', taskId: taskId, courseId: widget.courseId ?? '', grade: grade, date: DateTime.now(), notes: _notesCtrl.text.trim().isEmpty? null : _notesCtrl.text.trim());
...
class _EditGradeDialogState extends State<_EditGradeDialog> {
  late final TextEditingController _gradeCtrl = TextEditingController(text: widget.grade.grade.toString());
  late final TextEditingController _notesCtrl = TextEditingController(text: widget.grade.notes ?? '');
  @override
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: () async {
          ...
          await widget.svc.updateGrade(widget.grade.id, {'grade': grade, 'notes': _notesCtrl.text.trim().isEmpty? null : _notesCtrl.text.trim()});
          if (mounted) Navigator.pop(context);
        }, child: const Text('Guardar')),
      ],
    );
