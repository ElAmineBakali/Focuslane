  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<StudySession>>(
      ...
      builder: (context, snap) {
        if (!snap.hasData) return const Card(child: SizedBox(height: 130, child: Center(child: CircularProgressIndicator())));
        ...
        return Card(
          child: Column(
            children: [
              const ListTile(leading: Icon(Icons.timeline), title: Text('Horas totales por curso')),
              if (items.isEmpty)
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<StudySession>>(
      ...
      builder: (context, snap) {
        ...
        final items = map.entries.map((e) => MapEntry(e.key, e.value.length)).toList()..sort((a,b)=>b.value.compareTo(a.value));
        return Card(
          child: Column(
            children: [
              const ListTile(leading: Icon(Icons.calendar_today), title: Text('Días activos por curso')),
              if (items.isEmpty)
                const Padding(padding: EdgeInsets.only(bottom: 12), child: Text('Sin sesiones'))
              else
                ...items.map((e) => ListTile(
...
class _StreakCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<StudySession>>(
      ...
      builder: (context, snap) {
        if (!snap.hasData) return const Card(child: SizedBox(height: 120, child: Center(child: CircularProgressIndicator())));
        final list = snap.data!;
        final days = list.map((s) => DateTime(s.date.year, s.date.month, s.date.day)).toSet().toList()..sort();
        int streak = 0;
...
class _MinutesBarCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<StudySession>>(
      ...
      builder: (context, snap) {
        if (!snap.hasData) return const Card(child: SizedBox(height: 220, child: Center(child: CircularProgressIndicator())));
        final list = snap.data!;
        final now = DateTime.now();
        final map = <int, int>{}; // dayOffset -> minutes
        for (int i = 0; i < days; i++) { map[i] = 0; }
        for (final s in list) {
        ...
        for (int i = days - 1; i >= 0; i--) {
          bars.add(BarChartGroupData(x: days - 1 - i, barRods: [BarChartRodData(toY: (map[i] ?? 0).toDouble())]));
        }
        return Card(
          child: Padding(
...
class _MethodDistributionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<StudySession>>(
      stream: svc.streamSessions(courseId: courseId, limit: 500),
      builder: (context, snap) {
        if (!snap.hasData) return const Card(child: SizedBox(height: 180, child: Center(child: CircularProgressIndicator())));
        final list = snap.data!;
