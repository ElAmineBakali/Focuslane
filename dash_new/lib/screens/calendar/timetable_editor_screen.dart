...
class _TimetableEditorScreenState extends State<TimetableEditorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(t == null ? 'Nuevo horario' : 'Editar horario')),
      body: Column(
        children: [
          Padding(
                  ...
                  FilledButton.icon(
                    ...
                    onPressed: () async {
                      ...
                      final obj = Timetable(
                        ...
                        startHour: _start, endHour: _end, slotMinutes: _slot,
                      );
                      if (t == null) {
          ...
          if (widget.timetable != null) Expanded(child: _SlotsEditor(timetable: widget.timetable!)),
        ],
      ),
    );
  }
...
class _SlotsEditor extends StatelessWidget {
              ...
              child: ListView.separated(
                ...
                itemBuilder: (_, i) {
                  ...
                  return ListTile(
                    ...
                    title: Text(x.title),
                    subtitle: Text('${x.day} • ${x.start}–${x.end} • ${x.type.name}'),
                    trailing: IconButton(
  ...
  Future<void> _editSlot(
      BuildContext context, CalendarService svc, String timetableId, TimetableSlot? s) async {
          ...
          actions: [
            if (s != null)
              TextButton(
                ...
                onPressed: () async { await svc.deleteSlot(timetableId, s.id); if (ctx.mounted) Navigator.pop(ctx); },
              ),
            ...
            FilledButton(
              ...
              onPressed: () async {
                final obj = TimetableSlot(
                  ...
                  title: title.text.trim().isEmpty ? 'Bloque' : title.text.trim(),
                  type: type, note: note.text.trim().isEmpty ? null : note.text.trim(),
                );
                if (s == null) { await svc.addSlot(timetableId, obj); }
                else { await svc.updateSlot(timetableId, obj); }
