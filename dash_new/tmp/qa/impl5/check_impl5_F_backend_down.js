...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.resolve(__dirname, 'evidence_F_backend_down_error.png'), fullPage: true });

    const errorLine = logs.find((l) => l.includes('[FinanceReceiptAI] scan error message=')) || null;
    const hasError = !!errorLine && /No se pudo analizar el ticket/i.test(errorLine);
    const previewConfirmed = logs.some((l) => l.includes('[FinanceReceiptAI] preview confirmed'));

    await page.mouse.click(620, 240);
    await page.keyboard.press('Control+A');
    await page.keyboard.type('Manual backend caido');

    await page.mouse.click(620, 296);
    await page.keyboard.press('Control+A');
    await page.keyboard.type('9.80');
    ...
    console.log('RESULT_IMPL5_F_BACKEND_DOWN_START');
    console.log(JSON.stringify(result, null, 2));
    console.log('RESULT_IMPL5_F_BACKEND_DOWN_END');

    console.log('LOGS_IMPL5_F_BACKEND_DOWN_START');
    for (const line of logs.slice(-200)) console.log(line);
    console.log('LOGS_IMPL5_F_BACKEND_DOWN_END');
  } catch (error) {
    console.error('CHECK_IMPL5_F_BACKEND_DOWN_ERROR', error && error.stack ? error.stack : String(error));
    process.exitCode = 1;
  } finally {
    await browser.close();
  }
