...
import { caloriesFromPhotoOutputSchema, caloriesFromPhotoSchema, explainOrDebugSchema, financeClassifyOutputSchema, financeClassifySchema, receiptScanOutputSchema, receiptScanSchema, } from './schemas.js';
...
async function safePersistAiLog(input, route) {
    try {
...
const jsonOnlyRule = 'Return ONLY valid JSON. No markdown, no prose, no code fences, no comments. Keep keys exactly as requested.';
router.post('/v1/ai/finance/classify', async (req, res) => {
    ...
    try {
        ...
        const ai = await callOpenAiJson({
            ...
            userPrompt: `Classify this personal finance text for locale ${payload.locale}. ` +
...
router.post('/v1/ai/food/calories_from_photo', async (req, res) => {
    ...
    try {
        ...
        let ai = await callOpenAiJson({
            ...
            userPrompt: `Estimate calories and macros from this food photo for locale ${payload.locale}. ` +
        ...
        if (ai.data.confidence < 0.55) {
            ...
            ai = await callOpenAiJson({
                ...
                systemPrompt: `${jsonOnlyRule}\n` +
                    'Schema: {"estimatedCalories":number|null,"macros":{"protein":number|null,"carbs":number|null,"fat":number|null},"items":[{"name":string,"portion":string,"calories":number}],"confidence":number[0..1]}',
                userPrompt: `Re-evaluate this food photo for locale ${payload.locale}. ` +
...
router.post('/v1/ai/finance/receipt_scan', async (req, res) => {
    ...
    try {
        ...
        const ai = await callOpenAiJson({
            ...
            userPrompt: `Extract a structured receipt for locale ${payload.locale}. ` +
...
router.post('/v1/ai/sync/explain_or_debug', async (req, res) => {
